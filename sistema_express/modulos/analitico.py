...
def _extrair_texto_pdf(caminho):
    """Extrai texto de PDF de forma compatível com Android/APK.

    Usa pdftotext quando existir e pypdf como fallback interno.
    """
    caminho = Path(caminho)
    if not caminho.exists() or not caminho.is_file():
        raise RuntimeError(f'Arquivo PDF não encontrado: {caminho}')
    try:
        with open(caminho, 'rb') as f:
            f.read(5)
    except Exception as exc:
        raise RuntimeError(
            'O Android bloqueou a leitura do arquivo. Copie o PDF para Download, '
            'libere as permissões do app e tente escolher o arquivo listado.'
        ) from exc
    ...
    try:
        ...
        if texto:
            return texto
        raise RuntimeError('O PDF abriu, mas não retornou texto. Pode ser PDF escaneado/imagem.')
    except Exception as exc:
        raise RuntimeError(
            'Não foi possível ler o PDF informado. Motivo técnico: '
            f'{type(exc).__name__}: {exc}. '
            'Confirme se é PDF Tekinisa com texto selecionável e sem senha.'
        ) from exc
